...
parameters {
  ...
  real phi21; // Persistence of volatility of h1 in h2
  real<lower=0> sigma1;  // Standard deviation of y1 (white noise)
  real<lower=0> sigma2;  // Standard deviation of y2 (white noise)
  real<lower=-1,upper=1> rho; // Correlation factor
  vector[N] h1; //latent volatility vector of time series 1
  vector[N] h2; //latent volatility vector of time series 2
}
...
model {
  ...
  // Prior of correlation factor 
  rho ~ uniform(-1,1); 
  
  // Dynamics of the first latent volatility vector
  h1[1] ~ normal(mu1, sigma1/sqrt(1 - square(phi1)));
  for (t in 2:N) {
    h1[t] ~ normal(mu1 + phi1*(h1[t - 1] - mu1), sigma1);
  }
  
  // Dynamics of the second latent volatility vector with Granger Causality
  h2[1] ~ normal(mu2, sigma2/sqrt(1 - square(phi2)));
  for (t in 2:N) {
    h2[t] ~ normal(mu2 + phi2*(h2[t - 1] - mu2) + phi21*(h1[t - 1] - mu1), sigma2);
  }
  ...
  for (t in 1:N) {
    ...
    y2[t] ~ normal(mu2 + rho*exp(h2[t]/2)*exp(-h1[t]/2)*y1[t], sqrt((1 - square(rho)))*exp(h2[t]/2));
  }
...
generated quantities {
  ...
  for (t in 1:N){
    y_rep2[t] = normal_rng(mu2 + rho*exp(h2[t]/2)*exp(-h1[t]/2)*y1[t], sqrt((1 - square(rho)))*exp(h2[t]/2));
  }
